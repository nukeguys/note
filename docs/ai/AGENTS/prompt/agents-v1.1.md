...
모든 MUST / MUST NOT / ABSOLUTE 규칙에 WHY 필수 포함.
...
WHY: 번들 크기 300KB 제한 준수 (현재 280KB)
```
...
- 핵심 규칙 5~7개 이내로 요약 섹션 제공
- 나머지는 상세 섹션에 배치

## 7. AGENTS.md와 DECISIONS.md 관계

| 문서         | 역할                 | 내용                               |
| ------------ | -------------------- | ---------------------------------- |
| AGENTS.md    | "무엇을 해야 하는가" | 규칙, 원칙, 패턴                   |
| DECISIONS.md | "왜 그렇게 되었는가" | 기술 선택 이유, 대안, 트레이드오프 |

**원칙:**

- 기술 선택, 패턴 채택, 예외 허용의 근거는 AGENTS.md에 장황하게 작성하지 않음
- 반드시 DECISIONS.md로 연결
...
WHY: 상세 내용은 DECISIONS.md #003 참조
```
...
WHY: 팀 린터 규칙과 일치, 신규 팀원 혼란 방지

MUST NOT:

- 약어 사용 금지 (usr → user, btn → button)

WHY: 코드 검색 시 false negative 발생, 가독성 저하

예외:

- 업계 표준 약어는 허용 (HTML, API, URL)
```
